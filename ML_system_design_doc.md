...
Expected Loss Reduction и Chargeback Rate отдельно не выносятся в критерии успеха:
- Expected Loss Reduction является производной от Fraud Loss Rate.
- Chargeback Rate запаздывает по времени и подходит скорее для пост-анализа, чем для основного решения по A/B тесту.
- ROC-AUC и PR-AUC используются на offline-этапе отбора модели, но не являются критериями успеха online A/B теста.
