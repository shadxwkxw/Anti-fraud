...
1. Fraud Loss Rate (Потери от мошенничества / общий оборот)
2. Expected Loss Reduction (оценка экономии относительно control)
3. Chargeback rate
...
*Отказоустойчивость*
1) Без одной точки отказа: все ключевые компоненты (inference-сервис, feature store, decision engine) развёрнуты в нескольких инстансах с балансировкой нагрузки.
2) Ограничение по времени и повторы: если ML-инференс не укладывается в SLA (например, 100 ms), решение принимается через fallback — rule-based систему.
3) Мягкая деградация: при недоступности части данных (real-time features) используется упрощённый набор признаков или baseline-модель.
4) Защита от перегрузки: ресурсоёмкие операции (feature generation, batch scoring) выполняются асинхронно через очередь, чтобы не влиять на задуржку онлайн-сервиса.
5) DLQ (очередь ошибок): транзакции, которые не удалось обработать, отправляются в отдельную очередь для последующего анализа и повторной обработки.
6) Проверки здоровья: используются health-check’и и автоматическое исключение неработающих инстансов из балансировки.
7) Circuit Breaker: при росте ошибок ML-сервис временно отключается, а система автоматически переключается на fallback-логику.
8) Идемпотентность: повторная обработка одной и той же транзакции не приводит к дублированию решений или логов.
9) Мониторинг и алерты: система отслеживает задержку, error rate и availability, с автоматическими уведомлениями при деградации.
