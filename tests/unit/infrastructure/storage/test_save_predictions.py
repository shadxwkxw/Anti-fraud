...
def test_publish_missing():
    with patch(S3_DOWNLOAD, side_effect=FileNotFoundError("not in S3")):
        with pytest.raises(FileNotFoundError):
            save_predictions.publish_results("/tmp/nonexistent_test_abc.csv", "2024-01-01")
...
def test_publish_ok(tmp_path):
    ...
    with (
        ...
        save_predictions.publish_results(str(p), "2024-01-01")
...
def test_main(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["save.py", "--input", str(p), "--date", "2024-01-01"])
    with (
        patch("src.antifraud.infrastructure.storage.save_predictions.init_db"),
